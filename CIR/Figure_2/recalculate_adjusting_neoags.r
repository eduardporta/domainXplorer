...
for (i in 1:nrow.df) {
	...
	res3 <- anova (lm (ESTIMATE ~ Tissue + NumMuts + imm.data[,i+6], data = imm.data))
	res4 <- anova (lm (ESTIMATE ~ Tissue + Ratio + imm.data[,i+6], data = imm.data))
	...
	mat.res[i,4] <- as.numeric(as.character(res3$'Pr(>F)'[3]))
	mat.res[i,5] <- as.numeric(as.character(res4$'Pr(>F)'[3]))
}

colnames (mat.res) <- c ("Region", "domainXplorer", "AdjNeo", "AdjNumMut", "AdjRatio")
